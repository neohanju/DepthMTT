...
#define PARAM_TAG_KINECT   "KINECT"
...
// Calibration
#define PARAM_TAG_IMAGE_WIDTH               "image_width"
#define PARAM_TAG_IMAGE_HEIGHT              "image_height"
// Dataset
...
bool CDataManager::GetFrameImage(const int _camIdx, cv::Mat &_receiver, bool _clear, unsigned int *_frameIdx)
{
	...
	try
	{
		...
		arrFrameBufferFull_[_camIdx] = false || !_clear;
...
{	
		...
		attrIter++)
	{
		if (0 == std::string(PARAM_TAG_INPUT_SOURCE).compare(attrIter->name()))
		{
			...
			else if (strInputSource_.find(PARAM_TAG_KINECT) != std::string::npos)
			{
				stReadParamFrameGrabber.nInputSource = hj::KINECT;
				stReadParamFrameGrabber.bExistFrameRange = true;
			}
		...
		{			
			...
			else if (0 == std::string(PARAM_TAG_CALIBRATION).compare(nodeIter->name()))
			{
				for (pugi::xml_attribute_iterator attrIter = nodeIter->attributes_begin();
					...
					attrIter++)
				{
					if (0 == std::string(PARAM_TAG_IMAGE_WIDTH).compare(attrIter->name()))
					{
						nImageWidth_ = attrIter->as_int();
					}
					else if (0 == std::string(PARAM_TAG_IMAGE_HEIGHT).compare(attrIter->name()))
					{
						nImageHeight_ = attrIter->as_int();
					}
					//if (0 == std::string(PARAM_TAG_PATH).compare(attrIter->name()))
					//{
			...
			else if (0 == std::string(PARAM_TAG_DETECTION).compare(nodeIter->name()))
			{
					...
					attrIter++)
				{
					if (0 == std::string(PARAM_TAG_DETECTION_TYPE).compare(attrIter->name()))
					{
						...
						else if (0 == strDetectionType.compare("depth_head"))
						{
							stReadParamDetect2D.nDetectionType = hj::DEPTH_HEAD;
						}
					}
	...
	for (int camIdx = 0; camIdx < stViewInfos_.nNumCameras; camIdx++)
	{
		...
		vecTrack2DParams_[camIdx].nImageWidth  = nImageWidth_;
		vecTrack2DParams_[camIdx].nImageHeight = nImageHeight_;
